...
namespace LeetCode
{
    ...
    class FruitCollection
    {
        public class Solution
        {
            ...
            private int FillBasket(int index)
            {
                if (memo.ContainsKey(tree[index])) return memo[tree[index]];

                int count = 0;
                int fruit1 = tree[index];
                int fruit2 = -1;
                for (int i = index; i < tree.Length; i++)
                {
                    if (fruit2 == -1 && tree[i] != fruit1)
                    {
                        fruit2 = tree[i];
                    }
